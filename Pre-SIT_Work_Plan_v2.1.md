# Pre-SIT 容器化數據庫驗證工作計畫書（v2.1）

**文件版本**: v2.1  
**建立日期**: 2026年5月16日  
**最後更新**: 2026年5月16日  
**負責部門**: 應用架構團隊  
**變更說明**: 依據 v2.0 PoC（`presit-bdd-demo/poc/POC_RESULTS.md`，通過率 86%）回饋校準。修正 8 處與 upstream Spring PetClinic 真實行為不符之斷言、補齊缺失工件、釐清「Plan-faithful + upstream image」的相容範圍。  
...
| 組件 | 技術 | 用途 |
|------|------|------|
| 測試標的 | Spring PetClinic Microservices (含 config-server + discovery-server + 4 服務 + api-gateway，共 6 個 pod) | 完整微服務拓樸 |
| 容器平台 | Kind + Kubernetes | 本地 K8s 集群 |
| 鏡像倉庫 | Docker Registry (localhost:5000) | 自建私有倉庫，透過 containerd mirror 串接 |
| 部署工具 | ArgoCD | GitOps 自動同步 |
| BDD 框架 | Cucumber 7.15 + Gherkin (zh-TW) | 行為驅動測試 |
| 測試引擎 | JUnit 5 + Cucumber JUnit Platform Engine + AssertJ | 執行與斷言 |
| DB 驗證 | JDBC (PostgreSQL 16) + PreparedStatement | Schema / Data 驗證 |
| API 驗證 | Java HttpClient | REST 端點測試 |
| K8s 操作 | **`bitnami/kubectl:latest`**（`1.28` tag 已不存在於 Docker Hub） | initContainer 等待鏈 |
| 度量 | metrics-server (`--kubelet-insecure-tls`，Kind 必要參數) | Phase 2 資源使用量斷言 |

### 1.3 已知相容性約束（v2.1 新增）

PoC 證實「Plan-faithful + 不修改 upstream image」存在三項根本落差，必須在計畫面處理，無法靠配置繞過：

| # | 落差 | 原因 | v2.1 對策 |
|---|------|------|----------|
| **C1** | upstream PetClinic image 預設使用 **HSQLDB**，無 `postgres` profile | Phase 1 驗證的 Postgres ≠ 應用實際使用的 DB | 明確切割：Postgres 僅供 Phase 1 結構驗證；Phase 2/3/4 驗應用本身（HSQLDB in-memory）。若要端對端綁定須重 build PetClinic |
| **C2** | Spring Boot 3.2 + Spring Cloud Config + Eureka client 啟動穩態 ≈ **500–530 MiB**（即使 `-Xmx200m -XX:MaxMetaspaceSize=160m`） | JVM Metaspace + Direct Memory + Netty | Phase 2 記憶體門檻由 **512 MB → 768 MB**（見 §4 Phase 2） |
| **C3** | upstream actuator 預設 `show-details=never`；`components.*` 不存在。upstream customers-service `GET /owners/99999` 回 **200 + 空 body**，非 404 | upstream code 行為 | 移除 `components.db.*` 斷言、Phase 3 404 斷言改標記為 `@known-issue @skip`（見 §4 Phase 3） |
...
> ⚠️ v2.1：v2.0 的非標準 layout（`<testSourceDirectory>step-definitions</testSourceDirectory>` + runner 放在 `runners/`）導致 Surefire **0 test discovered**。改為標準 Maven layout。
...
│
├── src/test/java/com/presit/                ─── 標準 Maven 測試原始碼根目錄 ───
│   ├── runners/
│   │   └── PreSitTestRunner.java            JUnit 5 + Cucumber 入口
│   └── steps/
│       ├── DatabaseLayerSteps.java          Phase 1: JDBC + SQL 驗證
│       └── ApplicationIntegrationSteps.java Phase 2/3/4: HttpClient + kubectl + 性能 + 決策
│
├── src/test/resources/features/             ─── Gherkin 場景（business-readable）───
│   ├── database/01_database_layer.feature       Phase 1: DDL / DML / 約束 / 索引 / 引用完整性
│   ├── application/02_application_layer.feature Phase 2: Pod / Health / 連線池 / 日誌
│   ├── integration/03_integration_test.feature  Phase 3: CRUD / 跨服務 / 邊界值
│   └── e2e/04_e2e_go_nogo.feature               Phase 4: E2E / 性能基準 / Go-NoGo
│
├── sql/                                     ─── v2.1 新增 ─────────────────────
│   ├── 01-schema.sql                            7 張表 + 5 FK + 3 索引（對應 Phase 1 全部斷言）
│   └── 02-sample-data.sql                       筆數達門檻 + George Franklin 標準資料
│
├── manifests/                               ─── v2.1 新增 ─────────────────────
│   ├── 00-namespace.yaml
│   ├── 10-postgres.yaml                         StatefulSet + Service + ConfigMap initdb
│   ├── 20-config-server.yaml
│   ├── 30-discovery-server.yaml
│   ├── 40-microservices.yaml                    4 服務（JVM tune: -Xmx200m -XX:MaxMetaspaceSize=160m）
│   └── 50-presit-jobs.yaml                      4 Phase Jobs + RBAC + PVC + Secret
│
├── argocd/                                  ─── v2.1 新增 ─────────────────────
│   └── petclinic-pre-sit.yaml                   ArgoCD Application + PostSync hook
│
├── kind/                                    ─── v2.1 新增 ─────────────────────
│   ├── kind-config.yaml                         containerd mirror → kind-registry:5000
│   └── up.sh                                    冪等的環境啟動腳本
│
├── scripts/
...
### 3.1 Step Definition 必須避免的「假裝斷言」（v2.1 新增）

PoC 發現 v2.0 demo 中多個 step 看似綠燈但實際**不驗證任何東西**。Code review 須確認：

| 反例 (v2.0) | 正確做法 (v2.1) |
|------------|----------------|
| `result_should_be_empty()` 內 `List orphans = new ArrayList<>(); assertThat(orphans).isEmpty();` | WHEN step 把 ResultSet 讀入 `List<Map<String,String>>` field；THEN step 斷言該 list 為空 |
| `record_fields_should_match()` 只 `println()` | 對 ResultSet 第 1 列 `getString(field)` 與表格 value 比對 |
| `query_column_definitions()` 設了 print 但 `currentTableName` 從未存進 field | WHEN step 將 table name 存入 instance field，THEN step 用該 field 查 `information_schema.columns` |
| `should_return_n_records()` 不數 | WHEN step 同上讀入 list，THEN step 斷言 `hasSize(expected)` |
...
| 10 | 標準測試數據內容正確 | `@dml @sample-data` | George Franklin 欄位值比對（last_name/address/telephone，**不含 city**，因 owners 表已定義為 5 欄位 — v2.1 修正內部矛盾） |
| 11 | 自增序列正常運作 | `@ddl @sequences` | INSERT RETURNING id > 0，再 DELETE |

**Gherkin → Step 範例**：

> ⚠️ v2.1 注意：Gherkin zh-TW **不認得** `因為` 與 `否則`。v2.0 PoC 中兩個 feature 因此 fail FeatureParser。請改用 `# 註解` 或 `並且`。
...
| 2 | 微服務資源使用量合理 | `@startup @resource` | CPU < 80%, **MEM < 768 MB**（v2.1：原 512 MB 對 Spring Boot 3.2 不可達，見 §1.3 C2） |
| 3 | 健康檢查端點回應正常 | `@health @smoke` | /actuator/health → 200, "UP" |
| 4 | ~~數據庫健康指標正常~~ → **資料庫子端點存活** | `@health @db-indicator` | **v2.1：由 `components.db.status==UP` 改為 `GET /actuator/health/db → 200`**；upstream config 未開 `show-details=always`（見 §1.3 C3） |
| 5 | 連線池初始化成功 | `@connection-pool` | 活躍 > 0, 等待 = 0 |
| 6 | 啟動日誌中不包含錯誤 | `@logs` | 無 ERROR；**v2.1：「Connection refused」斷言改為僅檢查最後 30 行（steady state），因 api-gateway 啟動期 Netty/Eureka retry 必然出現** |
| 7 | K8s Service 端點解析正常 | `@k8s-service` | 5 個 Service 有就緒 Endpoint |
| 8 | **背景前置假設**「namespace 中 Pod 皆 Running」 | (背景) | **v2.1 新增：必須加上 `app!=presit-validation` label selector，否則會把驗證 Job 自己的 Pod 也算進去（PoC Phase 2 真實 fail 過）** |
...
| 9 | 查詢不存在的 Owner | `@error-handling @known-issue` | **v2.1：upstream PetClinic 對未知 owner 回 200 + 空 body（非 404）。標記為 `@known-issue`，CI 預設 `--tags "not @known-issue"`，待 upstream 修正前不計入分數（見 §1.3 C3）** |
...
| 條件 | 決策 |
|------|------|
| 通過率 ≥ 95%（排除 `@known-issue`）且無 `@critical` 場景失敗 | ✅ GO — 部署至 SIT |
| 通過率 < 95% 或 任何 `@critical` 場景失敗 | ❌ NO-GO — 修復後重新驗證 |
| Phase 1 失敗 | ❌ NO-GO 且 Phase 2/3/4 仍會嘗試執行（其 initContainer 以 `\|\| true` 容錯，便於同次取得完整失敗證據） |

> v2.1 修訂：v2.0「Phase 1 失敗則後續 Phase 不執行」改為「容錯執行以取得完整 evidence」，避免反覆 rerun。

**Phase 4 K8s Job 注意事項（v2.1 新增）**：

- 4 個 Job 共用一個 **PVC** (`presit-reports`)，Phase 4 才能彙整前三個 Phase 的 JUnit XML 算通過率
- Phase 4 的 `activeDeadlineSeconds` 必須 ≥ `Phase 3 的 activeDeadline + Phase 3 backoff 重試時間總和`，否則 Phase 4 容易在 initContainer 等待時被 K8s 殺掉（PoC 第一次嘗試即遇此問題）
- `imagePullPolicy: Always`（因使用 `:latest` tag，否則 IfNotPresent 會吃到舊 image cache）
...
### 6.1 驗證矩陣（v2.1：場景數依 PoC 實跑反推）

| 驗證層 | 邏輯場景 | scenario outline 展開後實例 | 涵蓋 Tag | 門檻 |
|--------|-------------|---|---------|------|
| Phase 1 數據庫層 | 11 | **17** | `@ddl @dml @constraints` | 全部通過 |
| Phase 2 應用層 | 8 (v2.1: +1 K8s service 背景) | **23** | `@startup @health @connection-pool @logs` | ≥ 90%（v2.1：原「全部通過」對 upstream 不切實際） |
| Phase 3 功能集成 | 12 (其中 1 `@known-issue` 不計分) | **12** | `@api @cross-service @error-handling` | ≥ 95%（排除 `@known-issue`） |
| Phase 4 端到端 | 5 | **5** | `@e2e @performance @go-nogo` | `@critical` 全部通過 |
| **合計** | **36** | **57** | | **≥ 95% 且無 `@critical` 失敗** |

### 6.1.1 v2.0 PoC 實跑成績（baseline，套用 v2.1 修訂前）

| Phase | tests | failures | 結果 |
|-------|-------|----------|------|
| Phase 1 | 17 | 0 | ✅ |
| Phase 2 | 23 | 6 | ❌ (74%) — 全屬 §1.3 C2/C3 |
| Phase 3 | 12 | 1 | ❌ (92%) — 屬 §1.3 C3 |
| Phase 4 | 5 | 0 | ✅ |
| **合計** | **57** | **7** | **86% → NO-GO** |

預期：套用 v2.1 修訂後（記憶體 768MB、移除 components.db、Phase 3 404 標 `@known-issue`），通過率應到 ≥ 95%，達 GO。

### 6.2 評分體系（100 分）

| 維度 | 配分 | 評分來源 |
|------|------|---------|
| Phase 1 數據庫層 | 25 分 | 17 例通過數 / 17 × 25 |
| Phase 2 應用層 | 20 分 | 23 例通過數 / 23 × 20 |
| Phase 3 功能集成 | 25 分 | (12 - `@known-issue` 數) 例通過數 × 25 |
| Phase 4 端到端 | 20 分 | 5 例通過數 / 5 × 20 |
| 文檔與可維護性 | 10 分 | 人工評審 |
...
| 風險 | 概率 | 影響 | 應對策略 |
|------|------|------|---------|
| Kind 集群資源不足 | 中 | 中 | 限制 Pod 資源上限；準備備用雲 K8s（6 個 PetClinic pod 共需 ~4 GB） |
...
| Job 串接失敗 | 中 | 高 | initContainer 超時設為 300s；backoffLimit=1（v2.1：原 2 太多重試，易吃 deadline） |
| 性能基準線過嚴 | 中 | 低 | Phase 4 先建立基準再逐步收斂 |
| **(v2.1 新增) Step Definition「假裝斷言」** | 高 | 高 | Code review checklist 必須涵蓋 §3.1 四種反例 |
| **(v2.1 新增) Phase 4 因前序 backoff 重試吃光 deadline 而被殺** | 中 | 高 | Phase 4 `activeDeadlineSeconds` ≥ Σ(前三 Phase deadline + 重試 buffer)；或拆 Phase 4 為獨立觸發 |
| **(v2.1 新增) `bitnami/kubectl:1.28` 已從 Docker Hub 下架** | 高 | 中 | 改用 `:latest` 或固定 sha256 |
| **(v2.1 新增) Gherkin zh-TW 非法關鍵字（`因為`/`否則`）導致 parser fail** | 高 | 高 | feature lint：CI grep 禁用詞，或改為 `# 註解` / `並且` |
| **(v2.1 新增) JVM 啟動峰值超過資源 limit** | 高 | 中 | `JAVA_TOOL_OPTIONS=-Xmx200m -XX:MaxMetaspaceSize=160m -XX:ReservedCodeCacheSize=48m`；container limit ≥ 768 MiB |
...
| **v2.1** | **2026-05-16** | **PoC 回饋校準**：(1) 新增 §1.3 相容性約束 C1/C2/C3；(2) Phase 1 `owners.city` 矛盾移除；(3) Phase 2 記憶體門檻 512→768 MB、`components.db.*` 改驗 `/actuator/health/db` 端點、Pod 背景檢查加 `app!=presit-validation` selector；(4) Phase 3 404 斷言改 `@known-issue`；(5) §3 改為標準 Maven layout；(6) §3.1 新增「假裝斷言」防止清單；(7) §5 補入缺失工件（SQL、Postgres manifest、4 服務 manifest、ArgoCD app、Phase 2-4 step bindings）；(8) §7 風險表加入 5 條 PoC 實證風險；(9) bitnami/kubectl tag 更新為 `:latest` |
