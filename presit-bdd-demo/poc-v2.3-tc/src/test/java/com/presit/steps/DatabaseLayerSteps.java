...
/**
 * v2.3-tc Phase 1：Testcontainers 版
 *
 * 架構差異（對比 v2.2）：
 *   v2.2  → 連線至 Kind pre-sit namespace 的 postgres K8s Service
 *   v2.3-tc → Testcontainers 在 Job Pod 內透過 Docker socket 自啟 PostgreSQL，
 *              再用 Flyway 執行三個 schema 的 migration，完全不依賴外部 DB pod。
 *
 * Kind 上執行需求（見 manifests/pre-sit-tc/30-bdd-jobs.yaml）：
 *   - 掛載 /var/run/docker.sock（hostPath）
 *   - 設定 TESTCONTAINERS_HOST_OVERRIDE=172.17.0.1（Docker bridge→host）
 *   - 設定 TESTCONTAINERS_RYUK_DISABLED=true（Ryuk 需 privileged，K8s 中停用）
 */
public class DatabaseLayerSteps {
    ...
    private static void runFlyway() {
        // 每個 microservice schema 獨立執行 Flyway，各自持有 flyway_schema_history
    ...
    public void verifyPostgresRunning(String ns) {
        // TC 版本：DB 由 Testcontainers 管理，不依賴 K8s namespace
        assertThat(postgres.isRunning()).as("TC PostgreSQL 應已啟動").isTrue();
        assertThat(connection).as("JDBC 連線應已建立").isNotNull();
    }
        ...
        for (String s : new String[]{"customers_schema", "vets_schema", "visits_schema"}) {
            try (Statement st = connection.createStatement();
                ...
                assertThat(rs.getInt(1))
                        .as("%s 應至少有 1 個 Flyway migration", s)
