...
> **本分支（`gcp-deployment`）** 是 `main`（Argo Workflows）的延伸，額外包含 §7.13 GKE 部署腳本（`scripts/setup-gcp.sh`）。如只需 Argo Workflows on Kind，請切換至 `main`；如需 Jenkins 替代方案，請切換至 `jenkins`。
...
└── presit-bdd-demo/                       BDD 專案目錄
    └── poc-v2.2/                          ⭐ v2.2 現行 BDD 專案（三 schema + Flyway + REST API 驗證）
```
