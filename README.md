...
7. [Quick Start（從零跑起 PoC）](#7-quick-start從零跑起-poc)
   ...
   - [7.5 Jenkins CI/CD](#75-jenkins-cicd-自動化在-kind-內)
...
graph LR
    ...
    subgraph 現行實作
        M[manifests/<br/>K8s YAML + Jenkins]
...
| CI/CD 整合（已實作 v2.3） | Jenkins 部署在 Kind 內；`manifests/jenkins/` + `Jenkinsfile` 已就緒，見 [§7.5](#75-jenkins-cicd-自動化在-kind-內) |
