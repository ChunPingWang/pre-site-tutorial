...
7. [Quick Start（從零跑起 PoC）](#7-quick-start從零跑起-poc)
   ...
   - [7.11 Argo Workflows：取代 Jenkins](#711-argo-workflows取代-jenkins)
...
graph LR
    ...
    subgraph 現行實作
        M[manifests/pre-sit-tc/<br/>K8s YAML + DinD（Phase 1 TC）]
        BDD[presit-bdd-demo/poc-v2.3-tc/<br/>Phase 1 Testcontainers BDD]
    end
...
| **操作指南** | | |
| **`README.md`（本檔）** ⭐ | 全貌導覽；§3–§6 設計原理；§7.1–§7.13 環境建置、CI/CD、Observability、Testcontainers 等 | 所有角色 |
...
| **PoC 驗證** | | |
| **[`presit-bdd-demo/poc-v2.3-tc/`](presit-bdd-demo/poc-v2.3-tc/)** ⭐ | Phase 1 Testcontainers BDD 專案（DinD + Flyway + Kind 驗證） | 動手跑或修改 |
...
| 了解 CI/CD、Observability、Testcontainers 等功能 | **README.md §7.5–§7.13** |
| 快速跑通 Testcontainers Phase 1 | [`presit-bdd-demo/poc-v2.3-tc/`](presit-bdd-demo/poc-v2.3-tc/)（§7.13 機制說明） |
...
```bash
bash scripts/setup-argo-workflows.sh
# 安裝完成後訪問：http://argo.local:30080（需先加入 /etc/hosts）
```
...
| 步驟 | 名稱 | 動作 | 預期輸出 |
|------|------|------|---------|
| 1 | preflight | 驗 kubectl 可用、namespace 存在、ArgoCD apps 存在 | `kubectl v1.36+` |
| 2 | reset-presit | 清舊 screenshot job + scale down report-server + 刪 PVC + 重啟 postgres-0 + rollout restart | `pod/postgres-0 condition met` |
| 3 | apply-bdd-jobs | git clone + apply 30-bdd-jobs.yaml + apply 40-ui-screenshot.yaml + scale up report-server | 4 BDD jobs + report-server created |
| 4 | wait-phase4 | Polling 每 15 秒，直到 phase4 Job `Complete` / `SuccessCriteriaMet` | `Phase 4 done: SuccessCriteriaMet` |
| 5 | ui-screenshot | 刪舊 Job + apply 41-ui-screenshot-job.yaml + 等完成 + 印 logs | HTML 報告產生於 `/reports/` |
| 6 | read-decision | 解析 phase4 logs 中的 JSON decision 欄位 | `"decision":"GO ✅"` |
| 7 | check-sit-state | 顯示 SIT namespace 4 個 deployment 的現行 image | `:sit-approved` |
